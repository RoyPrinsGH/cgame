...
namespace engine
{
    ...
    unsigned int loadTexture(const fastgltf::Asset& asset, const fastgltf::Image& image)
    {
        int width = 0;
        int height = 0;
        int channels = 0;
            ...
            fastgltf::visitor{
                ...
                [&](const fastgltf::sources::Array& source)
                {
                    pixels = stbi_load_from_memory(
                        ...
                        static_cast<int>(source.bytes.size()), &width, &height, &channels,
                        4);
                },
        ...
        const unsigned texture =
            rlLoadTexture(pixels, width, height, RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, 1);
    ...
    gpu_model loadModel(const std::filesystem::path& path, int max_instances)
    {
        ...
        model.instanceVboId = rlLoadVertexBuffer(
            initial.data(), static_cast<int>(initial.size() * sizeof(glm::mat4)), true);
        ...
        for (const auto& mesh : asset.meshes)
        {
            for (const auto& primitive : mesh.primitives)
            {
                ...
                gpuPrimitive.vertexVboId = rlLoadVertexBuffer(
                    vertices.data(), static_cast<int>(vertices.size() * sizeof(vertex)),
                    false);
                rlEnableVertexBuffer(gpuPrimitive.vertexVboId);
                rlSetVertexAttribute(0, 3, RL_FLOAT, false, sizeof(vertex),
                                     0); // position
                ...
                for (int column = 0; column < 4; column++)
                {
                    ...
                    rlSetVertexAttribute(location, 4, RL_FLOAT, false, sizeof(glm::mat4),
                ...
                if (primitive.materialIndex)
                {
                    ...
                    if (material.pbrData.baseColorTexture)
                    {
                        ...
                        if (gltfTexture.imageIndex)
                            gpuPrimitive.baseColorTextureId =
                                textures[*gltfTexture.imageIndex];
                    }
                }
    ...
    renderer::renderer()
    {
        ...
        const std::string fragmentShaderCode = readTextFile(
            "/home/rvne/development/cgame/cgame-client/shaders/default.frag");

        m_defaultShader =
            rlLoadShaderProgram(vertexShaderCode.c_str(), fragmentShaderCode.c_str());

        std::cout << m_defaultShader;

        m_defaultShaderViewMatrixLocation =
            rlGetLocationUniform(m_defaultShader, "matView");
        m_defaultShaderProjectionMatrixLocation =
            rlGetLocationUniform(m_defaultShader, "matProjection");
    ...
    {
        return glm::scale(glm::translate(glm::mat4(1.0f), instance.position) *
                        ...
                        const camera& camera) const
    {
        ...
        Matrix projection = MatrixPerspective(camera.fovY * DEG2RAD, aspectRatio,
                                              camera.nearPlane, camera.farPlane);

        Matrix view = MatrixLookAt(glmVecToRayMathVec(camera.position),
        ...
        matrices.push_back(
            makeTransform(getModelInstanceDataFromShip(gameState.m_playerShip), 0.1f));

        for (const auto& ship : gameState.m_enemyShips)
            matrices.push_back(makeTransform(getModelInstanceDataFromShip(ship), 0.1f));

        rlUpdateVertexBuffer(m_shipModel.instanceVboId, matrices.data(),
                             static_cast<int>(matrices.size() * sizeof(glm::mat4)), 0);
        ...
        for (const auto& primitive : m_shipModel.primitives)
        {
            ...
            rlSetUniform(m_defaultShaderBaseColorTextureLocation, &texture_slot,
                         RL_SHADER_UNIFORM_INT, 1);
            rlEnableVertexArray(primitive.vaoId);
            rlDrawVertexArrayInstanced(0, primitive.vertexCount,
                                       static_cast<int>(gameState.m_enemyShips.size()) +
                                           1);
            rlDisableVertexArray();
            rlDisableTexture();
        }
    ...
    void renderer::drawShip(world::ship ship)
    {
        ...
        // Quaternion raylibQuat = {shipRotation.x, shipRotation.y, shipRotation.z,
        // shipRotation.w}; instance.transform = QuaternionToMatrix(raylibQuat); auto
        // glmShipPosition = ship.position; DrawModel(instance, {glmShipPosition.x,
        // glmShipPosition.y, glmShipPosition.z}, 0.05f, WHITE);
    }
}